...
int8 Decoder::_read_AC(int &zz_idx) {
    int RS = -1;
    ...
    return (int8)(idx >= base ? idx : -(1<<S)+1+idx);
}
...
int Decoder::_read_n_bits(int n) {
    ...
    for (; n > 0; n -= 8) {
        ...
        if (left_len >= 0) {
            acc = (acc << n) | ((_hc_input >> left_len) & ((1<<n) - 1));
            _hc_i = left_len - 1;
        } else {
            acc = (acc << 8) | _hc_input;
            _hc_i = -1;
...
void Decoder::_read_entropy_block(uint8 J, double out_block[8][8]) {
    ...
    _hc_DC = &_hs[_DC_Huffman_selector[J]];
    _hc_AC = &_hs[_AC_Huffman_selector[J]];

    static uint8 block[8][8];
    ...
    for (int i = 0; i < 8; ++i)
        for (int j = 0; j < 8; ++j)
            imm_block[i][j] = (int16)block[i][j] * _quantization_tables[t][i][j];
                    ...
                    for (int x_cnt = 0; x_cnt < H_i[c]; ++x_cnt) {
                        ...
                        for (int dy = 0; dy < pxl_h[c] * 8; ++dy) {
                            for (int dx = 0; dx < pxl_w[c] * 8; ++dx) {
                                ...
                                cnls[c]->at(iy, ix) += block[dy/pxl_h[c]][dx/pxl_w[c]];
                            }
                        }
                    }
                }
